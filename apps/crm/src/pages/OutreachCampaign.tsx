...
import { Share2, Plus, Trash2, Send, AlertTriangle, CheckCircle, Smartphone, Users, HelpCircle, RefreshCw } from 'lucide-react';
...
export default function OutreachCampaign() {
  ...
  const handleSendWhatsApp = (phone: string) => {
    ...
    // Encode template
    const encodedText = encodeURIComponent(activeCampaign.template);
    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-slate-200 bg-white flex justify-between items-center flex-shrink-0">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage bulk WhatsApp outreach lists safely without Meta bans.</p>
        </div>
      ...
      <div className="p-8 pb-4 flex-shrink-0 space-y-6">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-4 items-start shadow-sm">
          ...
          <div>
            <h4 className="font-bold text-amber-900 text-sm">WhatsApp Safety Warning (Anti-Ban Protocol)</h4>
            <p className="text-amber-700 text-xs mt-1 leading-relaxed">
              Meta closely monitors business numbers for spam. To completely protect your account, we recommend sending 
              <strong> no more than 50–80 messages per day</strong>. Use the manual click-to-chat generator below to open chats, 
              which mimics standard user behavior and keeps your account safe.
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-hidden px-8 pb-8 flex gap-6">
        ...
        <div className="w-96 flex flex-col gap-6 flex-shrink-0 overflow-y-auto pr-1">
          {/* Numbers Import */}
          ...
          {/* Template Editor */}
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4 flex-1 flex flex-col min-h-[300px]">
            <h3 className="font-bold text-slate-900 text-sm">2. Outreach Message Template</h3>
            <textarea
              value={activeCampaign?.template || ''}
              onChange={(e) => handleUpdateTemplate(e.target.value)}
              placeholder="Outreach message goes here..."
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs leading-relaxed outline-none focus:border-blue-500 transition-all resize-none flex-1 font-medium text-slate-700"
            />
            <div className="text-[10px] text-slate-400 bg-slate-50 p-3 rounded-xl border border-slate-100">
              💡 Message links are created dynamically. Click any prospect send button to prefill this template directly on WhatsApp.
            </div>
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/20 border-b border-slate-100 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                    ...
                    <th className="py-3 px-6">Target Platform</th>
                  ...
                  {displayProspects.map((prospect, idx) => (
                    <tr key={prospect.phone} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-6 text-right">
                        <button
                          onClick={() => handleSendWhatsApp(prospect.phone)}
